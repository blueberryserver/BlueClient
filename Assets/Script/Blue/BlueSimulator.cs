    ...
    void Start()
    {
        ...
        for (int i = 0; i < row; i++)
        {
            for (int j = 0; j < col; j++)
            {
                ...
                UIFactory.Instance.CreateText(clientPanel.transform, "Text", "", rect, clientPanel.SetTextDisplay);

                // NetClient
                NetClient netClient = NetClientManager.Instance.CreateNetClient();
                NetClientManager.Instance.AddNetClient(key, netClient);
                ...
                // BlueMsgHanlder
                NetHanderInitializer.Instance.InitNetHandler(netClient, bot);
